...
async function ensureUserSchema() {
  ...
  await sql.query("CREATE UNIQUE INDEX IF NOT EXISTS users_username_unique ON users (LOWER(username)) WHERE username IS NOT NULL", []);
}

export async function getUserByEmail(email: string): Promise<MemberProfile | null> {
  await ensureUserSchema();
  const rows = await sql.query(
    `SELECT id, email, username, name, avatar, role, title, bio, tags, github, linkedin, website, commits, prs, lang, created_at
...
export async function updateUser(id: number, values: Partial<Pick<MemberProfile, "username" | "name" | "avatar" | "role" | "title" | "bio" | "tags" | "github" | "linkedin" | "website" | "lang">>) {
  await ensureUserSchema();
  const fields = Object.entries(values).filter(([, value]) => value !== undefined);
  if (!fields.length) return getUserById(id);
  const allowed = new Set(["username", "name", "avatar", "role", "title", "bio", "tags", "github", "linkedin", "website", "lang"]);
...
export async function deleteUser(id: number) {
  ...
  await sql.query("DELETE FROM projects WHERE user_id = $1", [id]);
...
export async function deleteProject(id: number, userId?: number) {
  await ensureProjectsTable();
  await sql.query(userId ? "DELETE FROM projects WHERE id = $1 AND user_id = $2" : "DELETE FROM projects WHERE id = $1", userId ? [id, userId] : [id]);
}
