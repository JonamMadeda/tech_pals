...
export default function MemberPage() {
  ...
  return <div className="min-h-screen overflow-hidden bg-[#f8fafc]"><header className="border-b border-slate-200 bg-white/85 backdrop-blur"><div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6"><Link href="/" className="font-mono text-lg font-bold text-slate-900"><span className="text-blue-500">&lt;</span>tech<span className="text-blue-600">_pals</span><span className="text-blue-500"> /&gt;</span></Link><div className="flex gap-2"><Link href="/discussions" className="rounded-lg border border-slate-200 px-3 py-2 font-mono text-xs text-slate-600 hover:bg-slate-50">discussions</Link><Link href="/projects" className="rounded-lg border border-slate-200 px-3 py-2 font-mono text-xs text-slate-600 hover:bg-slate-50">projects</Link></div></div></header>
    <main className="mx-auto max-w-6xl px-6 py-8"><div className="mb-6 flex flex-col justify-between gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end"><div><p className="font-mono text-xs font-semibold uppercase tracking-[0.14em] text-blue-700">Member area</p><h1 className="mt-2 text-2xl font-semibold tracking-[-0.025em] text-slate-900 sm:text-3xl">Welcome back, {profile.name.split(" ")[0]}.</h1><p className="mt-2 text-sm text-slate-600">Your projects, profile, and community meeting in one place.</p></div><div className="flex flex-wrap gap-2">{meeting?.meeting_url && <a href={meeting.meeting_url} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-md bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-800"><Video size={16} />Join today’s meeting</a>}<button onClick={() => setModal("project")} className="inline-flex items-center justify-center gap-2 rounded-md border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50"><Plus size={16} />New project</button></div></div>
