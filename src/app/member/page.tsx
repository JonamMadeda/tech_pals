...
type Meeting = { meeting_url: string; updated_at: string };
...
const inputStyle = "w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-primary-400 focus:bg-white";
...
export default function MemberPage() {
  ...
  const [workspaceTab, setWorkspaceTab] = useState<"projects" | "meetings">("projects");
  ...
  async function load() {
    try {
      const [profileResponse, projectResponse, meetingResponse] = await Promise.all([fetch("/api/profile"), fetch("/api/projects"), fetch("/api/meetings")]);
      if (profileResponse.status === 401) { window.location.href = "/login"; return; }
      if (!profileResponse.ok || !projectResponse.ok || !meetingResponse.ok) { setNotice("Could not load your workspace."); setLoading(false); return; }
      ...
      setMeeting(meetingData.meeting ?? null);
      setLoading(false);
    } catch {
  ...
  async function changePassword(event: FormEvent<HTMLFormElement>) {
    ...
    if (newPassword.length < 8) { setNotice("Password must be at least 8 characters"); return; }
    if (!/[A-Z]/.test(newPassword)) { setNotice("Password must contain at least one uppercase letter"); return; }
    if (!/[a-z]/.test(newPassword)) { setNotice("Password must contain at least one lowercase letter"); return; }
    if (!/[0-9]/.test(newPassword)) { setNotice("Password must contain at least one number"); return; }
    if (!/[^A-Za-z0-9]/.test(newPassword)) { setNotice("Password must contain at least one special character"); return; }
