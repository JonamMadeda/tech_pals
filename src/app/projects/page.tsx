...
function ProjectImage({ project, featured = false }: { project: Project; featured?: boolean }) {
  return project.image_url ? <img src={project.image_url} alt="" className={`w-full object-cover ${featured ? "h-52" : "h-40"}`} /> : <div className={`flex w-full items-center justify-center bg-slate-50 ${featured ? "h-52" : "h-40"}`}><FolderKanban size={featured ? 42 : 32} className="text-primary-400" /></div>;
}
...
function ProjectCard({ project, onReadMore, featured = false }: { project: Project; onReadMore: (p: Project) => void; featured?: boolean }) {
  ...
  return (
    <article className={`group flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-primary-200 hover:shadow-md ${featured ? "h-[360px]" : "h-[360px]"}`}>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {!featured && (
          <div className="flex items-start justify-between gap-3">
            ...
            <span className={`shrink-0 rounded px-1.5 py-0.5 font-mono text-[9px] font-bold uppercase ${project.project_url ? "bg-primary-50 text-primary-700" : "bg-slate-100 text-slate-600"}`}>{project.project_url ? "live" : "project"}</span>
          </div>
        )}
        ...
        <div className="relative mt-2 flex-1">
          ...
          {overflows && (
            <button onClick={() => onReadMore(project)} className="mt-1 font-mono text-xs font-bold text-primary-600 hover:text-primary-700">Read More</button>
          )}
        </div>
        <div className={`mt-auto ${featured ? "flex items-center justify-between pt-4" : ""}`}>
          {!featured && <div className="mb-3 flex flex-wrap gap-1.5">{project.tags?.map((tag) => <span key={tag} className="rounded border border-primary-100 bg-primary-50 px-2 py-0.5 font-mono text-[10px] text-primary-700">{tag}</span>)}</div>}
          {featured && <div className="flex flex-wrap gap-1">{project.tags?.slice(0, 3).map((tag) => <span key={tag} className="rounded bg-primary-50 px-2 py-0.5 font-mono text-[10px] text-primary-700">{tag}</span>)}</div>}
          {featured && project.project_url && <a href={project.project_url} target="_blank" rel="noreferrer" className="text-primary-600 hover:text-primary-700"><ArrowUpRight size={17} /></a>}
          {!featured && (
            <div className="flex items-center justify-between border-t border-slate-100 pt-3">
              <span className="font-mono text-[10px] text-slate-400">{new Date(project.created_at).toLocaleDateString()}</span>
              <div className="flex gap-3">
                {project.github_url && <a href={project.github_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-mono text-xs font-semibold text-slate-500 hover:text-primary-600"><Github size={14} />code</a>}
                {project.project_url && <a href={project.project_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-mono text-xs font-semibold text-slate-500 hover:text-primary-600"><ExternalLink size={14} />demo</a>}
    ...
    <div className="min-h-screen overflow-hidden bg-[#f8fafc]">
      ...
      <main className="mx-auto max-w-6xl px-6 py-14">
        <div className="mb-10 max-w-2xl">
          <span className="font-mono text-xs font-semibold tracking-widest text-primary-600">[ COMMUNITY_BUILDS ]</span>
        ...
        <section>
          <div className="mb-6 flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
            <div className="relative w-full lg:max-w-md">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search projects, people, or tools" className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 font-mono text-xs text-slate-800 outline-none focus:border-primary-400 focus:bg-white" />
            </div>
            <div className="flex flex-wrap gap-2">{tags.map((tag) => <button key={tag} onClick={() => setSelectedTag(tag)} className={`rounded px-2.5 py-1.5 font-mono text-[10px] font-semibold transition ${selectedTag === tag ? "border border-primary-200 bg-primary-50 text-primary-700" : "border border-slate-200 text-slate-500 hover:bg-slate-50"}`}>{tag.toLowerCase()}</button>)}</div>
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
              ...
              <Link href="/member" className="mt-5 inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-4 py-2.5 font-mono text-xs font-bold text-white hover:bg-primary-700">publish a project <ArrowUpRight size={14} /></Link>
            </div>
          ) : (
            <>
        ...
        {modalProject && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm" onClick={() => setModalProject(null)}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.15 }} className="relative max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="mt-4 flex flex-wrap gap-1.5">{modalProject.tags?.map((tag) => <span key={tag} className="rounded border border-primary-100 bg-primary-50 px-2 py-0.5 font-mono text-[10px] text-primary-700">{tag}</span>)}</div>
              ...
              <div className="mt-6 flex items-center gap-3 border-t border-slate-100 pt-4">
                ...
                {modalProject.project_url && <a href={modalProject.project_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-3 py-2 font-mono text-xs font-bold text-white hover:bg-primary-700"><ExternalLink size={14} />live demo</a>}
              </div>
            </motion.div>
          </motion.div>
        )}
