...
import { ArrowUpRight, Code2, ExternalLink, FolderKanban, Github, Search, SlidersHorizontal, Star } from "lucide-react";
          ...
          <section className="mb-12">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {featured.map((project) => (
                <article key={project.id} className="flex h-[360px] flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-slate-600">{project.summary || project.description || "Explore this community build."}</p>
                    <div className="mt-auto flex items-center justify-between pt-4">
                      ...
                      {project.project_url && <a href={project.project_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:text-blue-700"><ArrowUpRight size={17} /></a>}
                    </div>
                  </div>
                </article>
              ))}
            ...
            <>
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {visibleProjects.map((project) => (
                  <article key={project.id} className="group flex h-[360px] flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-md">
                    ...
                    <div className="flex flex-1 flex-col p-5">
                      ...
                      <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-slate-600">{project.summary || project.description || "Explore this community build."}</p>
                      <div className="mt-auto">
                        <div className="mb-3 flex flex-wrap gap-1.5">{project.tags?.map((tag) => <span key={tag} className="rounded border border-blue-100 bg-blue-50 px-2 py-0.5 font-mono text-[10px] text-blue-700">{tag}</span>)}</div>
