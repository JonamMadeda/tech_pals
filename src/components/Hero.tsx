    ...
    <section className="border-b border-slate-200 bg-[#fafaf8] px-6 pb-16 pt-32 md:pb-24 md:pt-40">
      <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
          <p className="mb-5 font-mono text-xs font-semibold uppercase tracking-[0.16em] text-primary-700">A community for people who make things</p>
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            <a href="/projects" className="inline-flex items-center gap-2 rounded-md bg-primary-700 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-primary-800 focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-offset-2">
              Explore projects <ArrowRight size={16} />
            </a>
            <button onClick={scrollToMembers} className="rounded-md border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition-colors hover:border-slate-400 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-offset-2">
              Meet the community
            </button>
          </div>
        </motion.div>
          ...
          <div className="mt-5 divide-y divide-slate-200 border-y border-slate-200">
            {highlights.map(({ icon: Icon, label, detail }) => (
              <div key={label} className="flex items-center gap-4 py-4">
                <span className="grid h-9 w-9 place-items-center rounded-md bg-primary-50 text-primary-700"><Icon size={17} /></span>
